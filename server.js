    ...
    fs.createReadStream(tempFilePath)
      ...
      .on('data', (row) => {
        ...
        if (!material || isNaN(weight) || weight < 0) {
          results.errors.push({ row: rowIndex, message: 'Puuttuva tai virheellinen materiaali tai paino', data: row });
          return;
        }
        if (!category) {
...
app.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const totalWeight = Object.values(result.totals).reduce((acc, cur) => acc + cur, 0);
    return res.json({
      success: true,
